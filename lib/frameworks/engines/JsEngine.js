...
export class JsEngine {
  ...
  instrument(content, fileId, extension) {
    ...
    } else if (['.js', '.jsx', '.ts', '.tsx', '.css', '.scss'].includes(extension)) {
      return content;
    }
    return content;
  }
  ...
  /**
   * Instruments Markdown content line-by-line.
   */
  instrumentMarkdown(content, fileId) {
    const lines = content.split('\n');
    return lines.map((line, i) => {
      // We tag each line with its number (1-indexed)
      return MarkerService.encode(fileId, i + 1) + line;
    }).join('\n');
  }

  /**
   * Instruments JSON content.
   * Simple implementation: we tag the start of the file.
   */
  instrumentJson(content, fileId) {
    // For JSON, we stick the marker at the beginning
    return MarkerService.encode(fileId, 1) + content;
  }
