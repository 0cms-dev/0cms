...
const server = http.createServer((req, res) => {
  ...
  // BUFFER REQUEST BODY for retries (crucial for POST/PUT during warm-up)
  let requestBody = [];
  let bodyGathered = false;
  if (req.method === 'POST' || req.method === 'PUT') {
      req.on('data', (chunk) => requestBody.push(chunk));
      req.on('end', () => { bodyGathered = true; });
  } else {
      bodyGathered = true;
  }

  const tryProxy = (attempt = 1) => {
    // Wait for body to be gathered before first attempt if it's a POST/PUT
    if (!bodyGathered) {
        setTimeout(() => tryProxy(attempt), 50);
        return;
    }
    ...
    const proxyReq = http.request(options, (proxyRes) => {
      ...
      // Force CORS/COEP/Keep-Alive headers on ALL proxied responses
      // Robustness: STRIP headers that frequently block iframing in local dev
      ...
      headers['Cross-Origin-Opener-Policy'] = 'unsafe-none'; // Better for iframe/host communication
    ...
    proxyReq.on('error', (e) => {
      // SILENT RECOVERY: Retry connection for up to 60 seconds (useful for Next.js cold boots)
      if (attempt < 120) { 
          if (attempt % 10 === 0) console.log('[bridge] Still waiting for dev server on port ' + TARGET_PORT + ' (Attempt ' + attempt + ')...');
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
        proxyReq.end(Buffer.concat(requestBody));
    } else {
        proxyReq.end();
    }
  };
...
server.on('error', (e) => {
  if (e.code === 'EADDRINUSE') {
    console.error('[Error] EADDRINUSE: Middleware port 3001 already in use.');
    process.exit(1);
  }
});
