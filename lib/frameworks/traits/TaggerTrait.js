...
export class TaggerTrait {
  ...
  async initWasm() {
    const success = await this.wasmEngine.init();
    if (success) {
        this.activeEngine = this.wasmEngine;
        console.log('[TaggerTrait] High-Performance WASM Engine activated.');
    }
    return success;
  }
  ...
  async instrumentBatch(files) {
    if (this.activeEngine === this.wasmEngine) {
        // High-speed WASM batch run
        return this.wasmEngine.instrumentBatch(files);
    }
    
    // Standard JS parallel run
    return Promise.all(files.map(f => this.instrumentFile(f)));
  }
