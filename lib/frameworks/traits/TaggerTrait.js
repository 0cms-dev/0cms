...
export class TaggerTrait {
  ...
  async initWasm() {
    try {
        ...
        console.warn('[TaggerTrait] WASM engine failed to load (falling back to JS):', e.message);
        return false;
    }
  }
       ...
       const dir = rawPath.startsWith('/') ? rawPath : '/' + rawPath;
        try {
          ...
          for (const file of files) {
            // SYNTAX SAFETY: Skip JS/TS/JSX files to prevent framework errors in Next.js
          ...
          // Silent skip for missing directories (expected in frameworks like Next.js)
        }
    }
  ...
  async instrumentBatch(pathList) {
    if (this.activeEngine === this.wasmEngine && WasmEngine.wasm) {
        const fileBatch = await Promise.all(pathList.map(async (path) => {
            const id = this.nextId++;
            this.fileMap.set(id, path);
            this.pathMap.set(path, id);
            ...
            const content = await this.wc.fs.readFile(path, 'utf8');
            const ext = '.' + path.split('.').pop();
            return { path, content, file_index: id, extension: ext };
        }));
  ...
  async stripBatch(files) {
    if (this.activeEngine === this.wasmEngine && WasmEngine.initialized) {
        return this.wasmEngine.stripBatch(files);
    }
  ...
  async recursiveReaddir(dir) {
    // 1. Get string array (WebContainer API default)
    const entries = await this.wc.fs.readdir(dir).catch(() => []);
    const files = [];

    // 2. Parallel stat() check for each entry
    await Promise.all(entries.map(async (name) => {
        const fullPath = (dir === '/' ? '' : dir) + '/' + name;
        try {
            const stat = await this.wc.fs.stat(fullPath);
            if (stat.isDirectory()) {
            ...
            // Skip unreadable/protected files
        }
    }));
