...
export class FrameworkBroker {
  ...
  async detect() {
    ...
    for (const driver of DRIVERS) {
        ...
        // Determine if this driver matches. 
        // We require ALL defined fingerprint components to match (Total-Fidelity).
        let isMatch = true;

        if (fp.dependencies && !fp.dependencies.some(d => deps[d])) isMatch = false;
        if (isMatch && fp.files && !fp.files.every(f => fileNames.includes(f))) isMatch = false;
        if (isMatch && fp.configFiles && !fp.configFiles.some(c => fileNames.some(f => f.startsWith(c)))) isMatch = false;
