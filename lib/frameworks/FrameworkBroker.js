...
export class FrameworkBroker {
  ...
  async detect() {
    ...
    // 3. Fallback: Universal Inference
    console.warn('[Broker] No specific framework driver matched. Inferring universal fallback...');
    ...
    if (fileNames.includes('package.json')) {
        ...
        this.activeDriver = {
            ...
            server: ServerTrait.from({ command: 'npm run dev', port: 3000 }), // Default assumption
            routing: RoutingTrait.from({
                contentPaths: [], // Forces global scan in WebContainerGitService
                extensions: ['.md', '.mdx', '.html', '.astro', '.vue', '.svelte', '.js', '.jsx', '.ts', '.tsx', '.json']
            }),
            content: ContentTrait.from({ fileTypes: ['.md', '.mdx', '.html'] }),
            templating: TemplatingTrait.from({ componentPath: '/src/components', templateType: 'html' })
        };
    } else {
        ...
        this.activeDriver = {
            id: 'universal-static',
            name: 'Static Website',
            server: ServerTrait.from({ command: 'npx --yes serve -p 3000', port: 3000 }),
            routing: RoutingTrait.from({
                contentPaths: ['/'],
                extensions: ['.html', '.md', '.js', '.css']
            }),
            content: ContentTrait.from({ fileTypes: ['.html', '.md'] }),
            templating: TemplatingTrait.from({ componentPath: '/', templateType: 'html' })
        };
    }
